...
export default function SupplierPortalRegisterPage() {
  ...
  async function handleRegister(e: FormEvent) {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: `${window.location.origin}/en/supplier-portal/auth/callback`,
      },
    });
    setLoading(false);
    if (error) {
    ...
    if (data.session) {
      router.push("/en/supplier-portal");
      router.refresh();
    } else {
      setCheckEmail(true);
    }
    ...
    <section className="px-6 py-16">
      <div className="max-w-sm mx-auto">
        ...
        <p className="text-sm text-slate-400 mb-8">
          Use the same email you registered your company with — we&apos;ll
          link your account to your company automatically. Haven&apos;t
          registered a company yet?{" "}
          <Link href="/en/suppliers/register" className="text-orange-400 hover:underline">
            Register here
          </Link>{" "}
          first.
        </p>
        ...
        <p className="text-sm text-slate-400 mt-8 text-center">
          ...
          <Link href="/en/supplier-portal/login" className="text-orange-400 hover:underline">
            Sign in
          </Link>
        </p>
      </div>
