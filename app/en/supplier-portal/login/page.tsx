...
export default function SupplierPortalLoginPage() {
  ...
  async function handleMagicLink(e: FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: { emailRedirectTo: `${window.location.origin}/en/supplier-portal/auth/callback` },
    });
    setLoading(false);
    if (error) {
      setError(error.message);
    ...
    <section className="px-6 py-16">
      <div className="max-w-sm mx-auto">
        ...
        <p className="text-sm text-slate-400 mb-8">
          Manage your products and view buyer opportunities.
        </p>
        ...
        <p className="text-sm text-slate-400 mt-8 text-center">
          ...
          <Link href="/en/supplier-portal/register" className="text-orange-400 hover:underline">
            Register
          </Link>
        </p>
      </div>
