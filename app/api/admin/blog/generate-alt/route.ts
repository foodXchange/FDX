...
export async function POST(req: NextRequest) {
  ...
  const { imageUrl, postTitle, postCategory } = body;
  ...
  const msg = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Write alt text for a hero image for this blog post:\nTitle: ${postTitle}\nCategory: ${postCategory ?? "food"}\n\nThe platform is FoodXchange — B2B kosher food sourcing for Israeli market.\n\nReturn only the alt text string. 80-120 characters.`,
      },
    ],
  });
