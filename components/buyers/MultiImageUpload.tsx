...
async function uploadFile(file: File): Promise<string> {
  ...
  const res = await fetch("/api/sourcing/upload-image?bucket=requests", { method: "POST", body: fd });
...
export default function MultiImageUpload({ value, onChange, maxImages = 5 }: Props) {
  ...
  // Allow parent to reset (e.g. after submit)
  useEffect(() => {
  ...
  function addUrl() {
    ...
    if (!canAdd) {
      setUrlError("Maximum 5 images reached");
      return;
    }
    setUrlError(null);
    ...
    <div className="space-y-3">
      {images.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm font-medium text-slate-700">Drop product images here</p>
