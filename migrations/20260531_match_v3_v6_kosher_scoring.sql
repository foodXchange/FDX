-- match_v3 v6 — restore kosher scoring to pts_compliance.
--
-- Change from v5:
--   pts_compliance now rewards kosher-certified suppliers when the buyer
--   requests kosher. This is a SCORING signal (max 10 pts), NOT a gate —
--   non-kosher suppliers still appear in results, they just rank lower.
--
--   Before (v5): kosher was fully neutralised — pts_compliance was certs-only.
--   After  (v6): kosher suppliers get 10 pts when kosher is required;
--                neutral 5 pts now requires BOTH no certs AND kosher_required=false.
--
-- Score max unchanged: 35 + 40 + 15 + 10 + 10 = 110.
...
-- kosher_required / kosher_type are derived here for the sourcing workflow
-- (buyer's cert request) and now also drive pts_compliance scoring (v6).
pip_c as (
  select
    pip.*,

    -- kosher level: used in pts_compliance scoring (v6) and workflow display
    case
...
candidates as (
  ...
  where so.status = 'approved'
    ...
    -- NOTE: kosher, kosher_passover, and halal gates REMOVED in v5 — unchanged in v6.
...
scored as (
  select
    ...
    -- pts_compliance (10 max)
    -- neutral 5 only when neither certs nor kosher are requested;
    -- kosher-certified suppliers score 10 when buyer requests kosher (scoring, not gating).
    (
      case
        when array_length(pip_c.req_certs, 1) is null
          and pip_c.kosher_required = false                         then 5
        when c.all_certs && pip_c.req_certs                         then 10
        when pip_c.kosher_required = true
          and c.kosher_types && array[pip_c.req_kosher_level]::text[] then 10
        else 0
      end
...
-- kosher/halal gates remain gone (v5 unchanged) — scoring only, never gating.
