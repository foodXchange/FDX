-- match_v3 v5 — kosher/halal neutralized as match gates and scores.
--
-- Domain insight: In Israeli food import, kosher status is UNKNOWABLE from a
-- supplier website. A factory may be certified but not advertising it, willing
-- to certify for a specific buyer, or uncertified — you learn which only after
-- emailing for a certificate. Excluding suppliers based on scraped kosher data
-- produces false negatives. kosher/halal are now POST-MATCH workflow flags only.
--
-- Changes from v4 (20260530_match_v3_v4.sql):
--
--   1. REMOVED kosher rank hierarchy gate from candidates WHERE.
--   2. REMOVED kosher-for-Passover gate from candidates WHERE.
--   3. REMOVED halal gate from candidates WHERE.
--   4. pts_compliance: kosher sub-score (0/5/10) removed. Certs-only, max 10.
--   5. pts_vector: multiplier 30→40, neutral 15→20. Redistributes freed 10 pts.
--      Total max remains 110 (category 35 + vector 40 + format 15 + compliance 10 + evidence 10).
--   6. Added kosher_status to breakdown jsonb (informational only — not scored):
--        'certified'  — kosher_level in (regular, mehadrin, badatz)
--        'not_listed' — kosher_level = 'none'
--        'unknown'    — kosher_level IS NULL (not scraped / not captured)
--
--   kosher_level / kosher_passover / is_halal COLUMNS are preserved — they still
--   store captured-if-present data and feed the kosher_status workflow flag.
--   The buyer's stated kosher requirement is still recorded in pip_c for the
--   sourcing workflow (so the admin knows to ask for a certificate), but it no
--   longer drives gating or scoring.
...
-- kosher_required / kosher_type are still derived here so the sourcing workflow
-- knows a buyer requires kosher (to ask suppliers for certificates). They no
-- longer drive gating or scoring.
pip_c as (
  select
    pip.*,

    -- kosher level (preserved for workflow — NOT used as a gate in v5)
    case
...
candidates as (
  ...
  where so.status = 'approved'
    ...
    -- NOTE: kosher, kosher_passover, and halal gates REMOVED in v5.
    -- A supplier's kosher/halal status is not reliably knowable from their website.
    -- Absence of a certification mention = UNKNOWN, never "not kosher/halal".
    -- These are verified post-match by requesting certificates directly from suppliers.
),
...
scored as (
  select
    ...
    -- pts_vector (40 max, neutral=20 when either embedding absent)
    -- Raised from 30→40 to amplify description similarity as the primary ranking signal.
    case
      when request_emb is not null and c.embedding is not null
        then greatest(0.0, (1.0 - (c.embedding <=> request_emb)) * 40.0)::numeric
      else 20.0
    end                                                             as pts_vector,
    ...
    -- pts_compliance (10 max — kosher sub-score removed in v5; certs overlap only)
    (
      case
...
select
  ...
  jsonb_build_object(
    ...
    'kosher_status', case
      when s.kosher_level in ('regular', 'mehadrin', 'badatz') then 'certified'
      when s.kosher_level = 'none'                             then 'not_listed'
      else                                                          'unknown'
    end
  )                                                                                 as breakdown,
...
-- Score max: category 35 + vector 40 + format 15 + compliance 10 + evidence 10 = 110.
-- kosher/halal gates are gone — all approved suppliers pass compliance check.
